...
Puppet::Reports.register_report(:upguard) do
  ...
  def process
    ...
    if status == 'changed'
      ...
      node_group_id = lookup_or_create_node_group(node_group_name)
  ...
  def lookup_or_create_node_group(node_group_name)
    if !node_group_name.nil? && !node_group_name.empty?
      ...
      node_group_id = upguard_node_group_create(API_KEY, APPLIANCE_URL, node_group_name)
  ...
  def determine_domain_details(node_name, node_os)
    ...
    if DOMAINS.is_a?(Array) && DOMAINS.any?
      DOMAINS.each do |domain|
        ...
        next unless node_name.end_with?(domain['name'])
  ...
  def upguard_node_group_create(api_key, instance, node_group_name)
